document.addEventListener('DOMContentLoaded', () => {
  ...
  // ✨ 엔딩 2 다단계 이미지 진행을 위한 변수 추가 (기존)
  let isEnding2 = false;
  let ending2Step = 0;
  
  // ----------------------------------------
  // ✨ [추가] 엔딩 4 다단계 이미지 진행을 위한 변수 추가
  let isEnding4 = false; // 현재 엔딩 4 진행 중인지
  let ending4Step = 0;   // 엔딩 4 현재 단계 (1부터 8까지)
  const MAX_ENDING4_STEP = 8; // 엔딩 4는 이미지가 8개이므로 별도 정의
  // ----------------------------------------
  ...
  function checkEnding() {
      if(totalClicks >= 10) {
          ...
          // 다단계 엔딩 플래그 초기화
          isEnding2 = false;
          isEnding4 = false; // ✨ [추가] 엔딩 4 플래그 초기화
          isEnding5 = false;
          ...
          if(onlinePercent > offlinePercent){
              ...
              if (counts.book > counts.door) {
                  ...
                  endingSrc = '../images/ending/ending3.png';
          ...
          // 다단계 엔딩 이미지 클릭 이벤트 재설정 (엔딩 2, 4, 5 모두 처리)
          endingImage.onclick = () => {
              ...
              } else if (isEnding4) { // ✨ [추가] 엔딩 4 진행 중
                  ending4Step++;
